...
with tab6:
     ...
     fig = px.scatter(airport_dist,x='MONTH',y='FLT_TOT_1',color='APT_NAME',title="Airport Traffic Distribution Drift", color_continuous_scale="Plasma")
     fig.update_traces(marker=dict(size=15, line=dict(width=2,color="rgba(255,255,255,0.4)")))
     fig.update_xaxes(showgrid=False)
     fig.update_yaxes(showgrid=False)
     ...
     fig.update_traces(marker=dict(size=40, line=dict(width=2,color="rgba(255,255,255,0.4)")))
     fig.update_xaxes(showgrid=False)
     fig.update_yaxes(showgrid=False)
     fig.update_layout(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)")     
     st.plotly_chart(fig, use_container_width=True)       
